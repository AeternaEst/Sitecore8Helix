  ...
  return facets.map(facet => {
    const mergedFacet: MergedFacet = {
      ...
      values: facet.values.map(facetValue => {
        const containsFacet = selectedFacets.find(
          selectedFacet => selectedFacet.facetKey === facet.key
        );
        const mergedFacetValue: MergedFacetValue = {
          ...
          selected:
            containsFacet &&
            containsFacet.facetValues.find(
              selectedFacetValue => selectedFacetValue === facetValue.key
            )
        };
